...
RUN_PROFILE <- "paper"
# Options:
#   "fast"  : quick pilot run.
#   "paper" : larger Monte Carlo, closer to the uploaded C file and paper setup.

GLOBAL_SEED <- 778
set.seed(GLOBAL_SEED)

OUTDIR <- Sys.getenv("RSM_OUTDIR", unset = file.path(getwd(), "endogenous_rsm_output"))
...
write.csv(summary_df, file.path(OUTDIR, "endogenous_rsm_summary.csv"), row.names = FALSE)

cat("\nAll simulations completed.\n")
cat("Summary saved to:", file.path(OUTDIR, "endogenous_rsm_summary.csv"), "\n")
cat("Scenario-level CSV and PNG files saved in:", OUTDIR, "\n")
