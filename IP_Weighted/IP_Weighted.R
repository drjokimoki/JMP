...
run_forecast <- function(monthly_path, quarterly_path, n_start = 40) {
  ...
  RSSS <- rand_subsample_from_df(
    ...
    B = rs_draws,
    weight_scheme = "abs_t",   # <-- turn on |t|-based weighting
...
saveRDS(res_list, file="/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_Weighted/final_list.RData")
...
out_dir <- "/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_Weighted/Shapley Value Decomposition"
...
plots <- vector("list", length(IP_leverage))

for (i in seq_along(IP_leverage)) {
  if(i==18){
    next
  }
  qtr <- as.character(IP_leverage[[i]][["summary"]][["vintage"]])
  
  df_i <- IP_leverage[[i]][[2]]                # ideally: res_list[[i]][["decomposition"]]
  stopifnot(all(c("Var","Shapley") %in% names(df_i)))
