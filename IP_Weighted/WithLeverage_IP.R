...
rand_subsample_from_df <- function(df, y_col, x_next, k, B = 1000,
                                   ...
                                   weight_scheme = c("uniform", "abs_t", "leverage", "hybrid"),
                                   alpha = 1,            # temperature for |t|-weights
                                   lev_eps = 1e-8,       # floor for probs
  ...
  # helper: compute |t|-stats for bivariate y~x (expanding, using all rows here)
  compute_abs_t <- function(vars) {
    t_stat <- sapply(vars, function(v) {
  ...
  # helper: compute column leverage scores on the predictor matrix
  # NOTE: column leverage uses *right* singular vectors V (not U).
  compute_leverage <- function(vars) {
    Xfull <- df[, vars, drop = FALSE]
    cc_all <- stats::complete.cases(Xfull)         # strict complete rows
    if (sum(cc_all) < 2) return(rep(NA_real_, length(vars)))
    
    Xm <- as.matrix(Xfull[cc_all, , drop = FALSE])
    
    # center columns; optionally standardize (to avoid scale domination)
    if (scale_predictors) {
      mu <- colMeans(Xm, na.rm = TRUE)
      sdv <- apply(Xm, 2, sd)
      sdv[!is.finite(sdv) | sdv == 0] <- 1
      Xm <- scale(Xm, center = mu, scale = sdv)
    } else {
      Xm <- scale(Xm, center = TRUE, scale = FALSE)
    }
    
    sv <- tryCatch(svd(Xm), error = function(e) NULL)
    if (is.null(sv) || is.null(sv$v)) return(rep(NA_real_, length(vars)))
    
    # effective numerical rank
    tol <- max(dim(Xm)) * .Machine$double.eps * ifelse(length(sv$d)>0, sv$d[1], 1)
    r <- sum(sv$d > tol)
    if (r == 0) return(rep(NA_real_, length(vars)))
    
    V <- sv$v[, 1:r, drop = FALSE]        # right singular vectors (cols = components)
    # column leverage for variable j = ||V[j, 1:r]||^2
    lev <- rowSums(V^2)
    as.numeric(lev)
  }
  
  if (weight_scheme == "abs_t") {
    abs_t <- compute_abs_t(cand)
    w <- exp(alpha * abs_t)
    w[!is.finite(w)] <- 0
    sw <- sum(w, na.rm = TRUE)
    prob_vec <- if (sw > 0) w / sw else rep(1/length(cand), length(cand))
    
  } else if (weight_scheme == "leverage") {
    lev <- compute_leverage(cand)
    if (all(!is.finite(lev)) || sum(lev, na.rm = TRUE) <= 0) {
      warning("Leverage scores could not be computed reliably; falling back to uniform sampling.")
      prob_vec <- rep(1/length(cand), length(cand))
    } else {
      lev[!is.finite(lev) | lev < 0] <- 0
      prob_vec <- lev + lev_eps
      prob_vec <- prob_vec / sum(prob_vec)
    }
    
  } else if (weight_scheme == "hybrid") {
    # combine leverage and |t| via multiplicative weighting
    lev <- compute_leverage(cand)
    abs_t <- compute_abs_t(cand)
    if (all(!is.finite(lev))) lev <- rep(1, length(cand))
    lev[!is.finite(lev) | lev < 0] <- 0
    lev <- lev + lev_eps
    w_t <- exp(alpha * abs_t); w_t[!is.finite(w_t)] <- 0
    w <- lev * w_t
    sw <- sum(w, na.rm = TRUE)
    prob_vec <- if (sw > 0) w / sw else rep(1/length(cand), length(cand))
  }
...
run_forecast <- function(monthly_path, quarterly_path, n_start = 40) {
  ...
  Monthly_FRED_clean <- Monthly_FRED[ , -which(names(Monthly_FRED) %in% c("INDPRO"))]
  ...
  y_levels <- tibble(
    ...
    INDPRO_level = suppressWarnings(as.numeric(as.character(
      Quarterly_raw_for_levels[-1, which(names(Quarterly_raw_for_levels)=="INDPRO"), drop = TRUE]
    )))
  ) %>% arrange(Date) %>%
    drop_na()
  
  # (Optional) Sanity check: code for INDPRO expected to be 5 (Δlog)
  gdp_code <- suppressWarnings(as.integer(Quarterly_FRED_clean[1, "INDPRO"]))
  if (!is.na(gdp_code) && gdp_code != 5) {
    warning(sprintf("INDPRO transform code is %s, not 5. Back-transform may be inappropriate.", gdp_code))
  }
  ...
  Final_quarterly <- Final_quarterly[c("INDPRO","Date")] %>% arrange(Date)
  ...
  y <- estimation0$INDPRO
  X <- estimation0 %>% select(-Date, -INDPRO)
  ...
  fcasts <- univariate_forecasts_split(
    y, X_plus, y_levels = y_levels$INDPRO_level[-1], n_start = n_start
  )
  ...
  # Align growth_y
  growth_y <- (y_levels$INDPRO_level / dplyr::lag(y_levels$INDPRO_level) - 1)*100
  growth_y <- growth_y[-1]
  ...
  RSSS <- rand_subsample_from_df(
    ...
    B = rs_draws,
    weight_scheme = "leverage",   # <-- turn on |t|-based weighting
...
saveRDS(res_list, file="/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_Weighted/final_list_leverage.RData")
...
growth_y <- (ak$INDPRO/ dplyr::lag(ak$INDPRO) - 1)*100
growth_y <- growth_y[-1]
growth_y <- as.data.frame(growth_y)
...
for(i in 1:length(res_list)){
  ...
  #unpack a 3D array
  mat <-res_list[[i]][[2]]
  form = sprintf('IP_Weighted_Shapley_%s.csv', ae)
  write.csv(mat, file = form)
  
}

out<-as.data.frame(out[-18,])
write.csv(summary_results, "/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_uniform_forecast_error.csv")
write.csv(out, "/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_UNIFORM_RS_predi_for_histograms.csv")



asd<-read.csv("/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/uniform_forecast_error.csv", stringsAsFactors = FALSE, check.names = FALSE)

mean(abs(asd$MEAN_mean-asd$actual))
mean(abs(asd$RS_mean-asd$actual))
sqrt(mean((asd$MEAN_mean-asd$actual)^2))
sqrt(mean((asd$RS_mean-asd$actual)^2))


mean(abs(asd$MEAN_mean[-c(83,84)]-asd$actual[-c(83,84)]))
mean(abs(asd$RS_mean[-c(83,84)]-asd$actual[-c(83,84)]))
sqrt(mean((asd$MEAN_mean[-c(83,84)]-asd$actual[-c(83,84)])^2))
sqrt(mean((asd$RS_mean[-c(83,84)]-asd$actual[-c(83,84)])^2))
...
mean(abs(summary_results$MEAN_mean[-c(83,84)]-summary_results$actual[-c(83,84)]))
mean(abs(summary_results$RS_mean[-c(83,84)]-summary_results$actual[-c(83,84)]))
sqrt(mean((summary_results$MEAN_mean[-c(83,84)]-summary_results$actual[-c(83,84)])^2))
sqrt(mean((summary_results$RS_mean[-c(83,84)]-summary_results$actual[-c(83,84)])^2))
...
df<-aq
plot_top10_df <- function(df, n = 10, other_uses_abs_sum = FALSE, title = NULL) {
  stopifnot(all(c("Var", "Shapley") %in% names(df)))
  df <- df %>% drop_na(Shapley)
  
  top <- df %>% slice_max(order_by = abs(Shapley), n = n, with_ties = FALSE)
  
  if (nrow(df) > nrow(top)) {
    rest <- anti_join(df, top, by = "Var")
    other_sum <- if (other_uses_abs_sum) sum(abs(rest$value)) else sum(rest$value)
    final <- bind_rows(top, tibble(name = "Other", value = other_sum))
  } else {
    final <- top
  }
  ...
  top<-top[,c(1,6)] 
  top[n+1,]<-c("Other", sum(rest$Shapley))
  top$Shapley<-as.numeric(top$Shapley)
  ggplot(top, aes(x = fct_reorder(Var, Shapley, .desc = T), y = Shapley)) +
    geom_col() +
    coord_flip() +
    labs(x = NULL, y = "Contribution",
         title = title %||% sprintf("Shapley Value Decomposition")) +
    theme_minimal(base_size = 12)
}

## Example
# df <- tibble(name = c("A","B","C",...), value = c(...))
plot_top10_df(aq, n = 10, other_uses_abs_sum = FALSE)
...
for (i in 1:length(res_list)) {
  ...
  ggsave(filename = fn, plot = p, width = 7, height = 5, dpi = 300, path = "/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_Weighted/Shapley Value Decomposition")
}
?ggsave


out_dir <- "/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_Weighted/Shapley Value Decomposition Leverage"
...
for (i in seq_along(res_list)) {
  ...
  qtr <- as.character(res_list[[i]][["summary"]][["vintage"]])
  
  df_i <- res_list[[i]][[2]]                # ideally: res_list[[i]][["decomposition"]]
  stopifnot(all(c("Var","Shapley") %in% names(df_i)))
  ...
  ggsave(
    filename = fn,
    plot = p,
    width = 7, height = 5,                # optional; remove if cairo isn't available
    path = out_dir
  )
}
dev.off()
