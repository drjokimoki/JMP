...
/* =========================
   Σ_u builder — CENTERED, UNBIASED sample covariance of forecast errors
   ========================= */
/* For each origin (training window length T_tr), we compute
   u_{ti} = y_t - gamma_i * x_{ti}, then
   u_mean[i] = (1/T_tr) sum_t u_{ti},
   Sigma_u(i,j) = (1/(T_tr-1)) sum_t (u_{ti}-u_mean[i]) (u_{tj}-u_mean[j]).
   Stored in lower-triangular form inside an N x N array.
*/
static void build_sigma_u_lower_centered(const double *X, const double *y_tr,
                                         ...
                                         double *Sigma_u_lower)
{
    ...
    // 1) compute means of residuals per model
    double *u_mean = (double*)calloc((size_t)N, sizeof(double));
    for (int t=0; t<T_tr; t++){
        const double *xt = X + (size_t)t*N;
        double yt = y_tr[t];
        for (int i=0; i<N; i++) u_mean[i] += (yt - gamma[i]*xt[i]);
    }
    double invT = 1.0 / (double)T_tr;
    for (int i=0; i<N; i++) u_mean[i] *= invT;

    // 2) accumulate centered cross-products
    for (int t=0; t<T_tr; t++){
        const double *xt = X + (size_t)t*N;
        double yt = y_tr[t];
        for (int i=0; i<N; i++){
            double ui = yt - gamma[i]*xt[i] - u_mean[i];
            for (int j=0; j<=i; j++){
                double uj = yt - gamma[j]*xt[j] - u_mean[j];
                Sigma_u_lower[i*N + j] += ui * uj;
            }
        }
    }

    // 3) scale by (T_tr - 1) for unbiased sample covariance
    double scale = 1.0 / (double)(T_tr - 1);
    for (int i=0;i<N;i++) for (int j=0;j<=i;j++) Sigma_u_lower[i*N+j] *= scale;

    free(u_mean);
}

/* =========================
   Sum-to-one optimal weights from covariance (lower-triangular input)
   ========================= */
        ...
        for (int zi=0; zi<zc; zi++){
            ...
            for (int repl=0; repl<n_rep; repl++){
                ...
                for (int k=1; k<=steps; k++){
                    ...
                    /* --- Build Σ_u (CENTERED, UNBIASED) on training window --- */
                    build_sigma_u_lower_centered(X, y_tr, gamma, T_tr, N, Sigma_u_lower);
