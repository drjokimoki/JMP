/*
  ...
  + STREAMING prints: baseline first, then for each z: UNIFORM then WEIGHTED

  Build:
    gcc -O3 -march=native -ffast-math -funroll-loops -DUSE_OMP FInal_sum_up_one_weighted.c -o runme-lm
*/
...
/* =========================
   NEW: |t|-stats on y ~ yhat_i and softmax weights
   ========================= */
...
int main(void){
    ...
    for (int s=0; s<num_scen; s++){
        ...
        /* ===== RS by z; for each z print UNIFORM then WEIGHTED ===== */
