/*
  ...
  + Single subset size z = round(sqrt(T_len)) (clamped to [2, N])
  + RS can be UNWEIGHTED (uniform subsets) or |t|-WEIGHTED via softmax(exp(alpha*|t|))
  + STREAMING prints: baseline first, then UNIFORM and WEIGHTED at that z

  Build:
    gcc -O3 -march=native -ffast-math -funroll-loops -DUSE_OMP FInal_sum_up_one_weighted_het.c -o runme-lm -lm
*/
...
static const int T_len_list[] = {250};
static const int T_len_list_len = 2;
static const int N = 50;
static const double sigma_e = 1.0;
static const int K_RS = 1000;
static const double rho_list[] = {0.3};
static const int rho_list_len = 2;
static const double R2_list[] = {0.5, 0.3, 0.1};
static const int R2_list_len = 3;
static const int OOS_LEN_FIXED = 20;      /* fixed 20 OOS points */
static const double ridge = 1e-10;

/* --- |t|-weights hyperparam --- */
static const double T_ALPHA = 1.0;        /* alpha in exp(alpha*|t|) */

/* --- NEW: Heteroskedasticity knobs ---------------------------------- */
/* Cross-sectional heteroskedasticity in predictor idiosyncratic eta_t:
   Σ_eta = D * Toeplitz(rho) * D, where D=diag(eta_sd[i]).
   Turn this ON to make |t|-stats more informative and help |t|-weighted RS.
*/
static const int    USE_HET_X      = 0;    /* 1=on, 0=off */
static const double ETA_SD_STRONG  = 0.3;  /* i=0..4   (β strong) */
static const double ETA_SD_WEAK    = 1.0;  /* i=5..9   (β weak)   */
static const double ETA_SD_PI      = 2.0;  /* i=10..19 (β pi)     */
static const double ETA_SD_NULL    = 3.0;  /* i=20..49 (β zero)   */

/* Optional time-varying heteroskedasticity in u_t (common to all predictors).
   Var(u_t) = sigma_e^2 * (1 + U_HET_ALPHA * F_t^2).
   This changes overall difficulty but does NOT by itself separate |t| across predictors,
   so it is OFF by default.
*/
static const double U_HET_ALPHA    = 0.0;  /* set to e.g. 0.5 to enable */
...
/* ---------- NEW: heteroskedastic u_t with factor-based variance ------- */
static void gen_u_het_factor(int T, const double *F, double sigma_e2, double alpha, xrng *r, norm_state *st, double *u){
    u[0] = 0.0;
    for (int t=1; t<T; t++){
        double v = sigma_e2 * (1.0 + alpha * F[t]*F[t]);
        if (v < 1e-12) v = 1e-12;
        double sd = sqrt(v);
        u[t] = sd * xrng_normal(r, st);
    }
}
...
/* =========================
   |t|-stats on y ~ yhat_i and softmax weights
   ========================= */
static void compute_pred_based_tabs(const double *X, const double *y_tr,
                                    const double *gamma, int T_tr, int N,
                                    double *tabs_out)
{
    for (int i=0;i<N;i++){
        ...
        for (int t=0;t<T_tr;t++){
            double yhat = gamma[i] * X[(size_t)t*N + i];
            double y    = y_tr[t];
            Sxx += yhat*yhat;
            Sxy += yhat*y;
        }
        ...
        if (Sxx > 0.0 && T_tr > 1){
            ...
            double sigma2 = SSE / (double)(T_tr - 1);
            double se = sqrt(sigma2 / Sxx);
            if (se > 0.0) tabs = fabs(b / se);
        }
        tabs_out[i] = tabs;
    }
}

/* Weighted sampling w/o replacement (Efraimidis–Spirakis). */
typedef struct { double key; int idx; } KeyIdx;
static int cmp_keydesc(const void *a, const void *b){
    const KeyIdx *ka = (const KeyIdx*)a, *kb=(const KeyIdx*)b;
...
static void sample_k_weighted_no_replace(xrng *r, int n, int k, const double *weights, int *sub){
    int anypos = 0;
    for (int i=0;i<n;i++){ if (weights[i] > 0.0){ anypos = 1; break; } }
    if (!anypos){ int pool[256]; sample_k_no_replace(r, n, k, pool, sub); return; }
    ...
    for (int i=0;i<n;i++){
        ...
        if (w <= 0.0){
            ...
            double U = ((xrng_next(r) >> 8)+1.0)/16777217.0; /* (0,1] */
            keys[i].key = exp(log(U) / w); /* A-ExpJ */
        }
        keys[i].idx = i;
    }
    qsort(keys, n, sizeof(KeyIdx), cmp_keydesc);
    for (int i=0;i<k;i++) sub[i] = keys[i].idx;
}
...
int main(void){
    ...
    /* open CSV and write header (extended with hetero knobs) */
    FILE *fp = fopen("rs_results.csv","w");
    if (!fp){ perror("fopen rs_results.csv"); return 1; }
    fprintf(fp,
        "T_len,rho,R2_target,phi_f,phi_eta,phi_u,z,rs_mode,"
        ...
        "USE_HET_X,ETA_SD_STRONG,ETA_SD_WEAK,ETA_SD_PI,ETA_SD_NULL,U_HET_ALPHA\n");
    fflush(fp);
    ...
    /* beta_raw: first 5 strong, next 5 weak, next 10 = pi, others zero */
    const double b_strong = 100.0;
    const double b_weak   = 0.0;
    const double b_pi     = 0.0;
    double beta_raw[N];
    for (int i=0;i<N;i++){
        if (i < 5)        beta_raw[i] = b_strong;
        else if (i < 10)  beta_raw[i] = b_weak;
        else if (i < 20)  beta_raw[i] = b_pi;
        else              beta_raw[i] = 0.0;
    }
    ...
    const char *mode_names[2] = {"uniform","tsoft"}; /* print & CSV order: uniform then weighted */
    ...
    for (int s=0; s<num_scen; s++){
        ...
        const double phi_u = scen[s].phi_u; /* reported but unused for u_t (iid baseline) */
        ...
        /* choose a single z = round(sqrt(T_len)), clamped to [2, N] */
        int current_z = (int)floor(sqrt((double)T_len) + 0.5);
        if (current_z < 2) current_z = 2;
        if (current_z > N) current_z = N;

        /* ===== Build predictor idio covariance Σ_eta with heteroskedastic scales ===== */
        double *Sigma_base = (double*)malloc(sizeof(double)*N*N);
        toeplitz_cov(rho, N, Sigma_base);
        ...
        for (int i=0;i<N;i++){
            if (!USE_HET_X){
                ...
                if (i < 5)        eta_sd[i] = ETA_SD_STRONG;
                else if (i < 10)  eta_sd[i] = ETA_SD_WEAK;
                else if (i < 20)  eta_sd[i] = ETA_SD_PI;
                else              eta_sd[i] = ETA_SD_NULL;
            }
        }
        ...
        for (int i=0;i<N;i++){
            for (int j=0;j<N;j++){
                Sigma[i*N+j] = eta_sd[i] * Sigma_base[i*N+j] * eta_sd[j];
            }
        }
        ...
        /* ===== Baseline (equal-weight mean of univariate forecasts) ===== */
        ...
        for (int repl=0; repl<n_rep; repl++){
            ...
            /* u_t: hetero optional */
            if (U_HET_ALPHA > 0.0){
                gen_u_het_factor(T_len, F_t, sigma_e*sigma_e, U_HET_ALPHA, &r, &st, u_t);
            } else {
        ...
        printf("SCEN %d/%d  T=%d rho=%.2f R2=%.2f phis=(%.1f,%.1f,%.1f)  |  using z=%d\n",
               s+1, num_scen, T_len, rho, R2_target, phi_f, phi_eta, phi_u, current_z);
        printf("  Baseline RMSFE(avg): %.6f  |  RMSE(pooled): %.6f  |  bias: %.6f  var: %.6f\n",
               rmsfe_base, rmse_pooled_base, bias2_base, var_base);

        /* ===== RS at a single z = round(sqrt(T_len)) ===== */
        for (int mode=0; mode<2; mode++){
            const int use_weighted = (mode==1); /* 0: uniform, 1: |t|-softmax */
            ...
            for (int repl=0; repl<n_rep; repl++){
                /* ================= RNG SPLIT (data vs selection) ================= */
                xrng r_data, r_sel;
                norm_state st = {0,0.0};
                unsigned seed = MAIN_SEED ^ (unsigned)(s*1315423911u) ^ (unsigned)(repl*2654435761u);
                xrng_seed(&r_data, seed);
                unsigned seed_sel = seed ^ (0x9E3779B9u * (unsigned)(1 + use_weighted));
                xrng_seed(&r_sel, seed_sel);
                /* ================================================================== */
                ...
                /* DGP (use r_data) */
                gen_AR1_scalar_innovvar(T_len, phi_f, 1.0, &r_data, &st, F_t);
                gen_E_AR1_innovcov(T_len, N, phi_eta, Sigma_chol, &r_data, &st, E_t);
                for (int t=0;t<T_len;t++){
                    const double Ft = F_t[t];
                    double *xrow = X + (size_t)t*N;
                    const double *erow = E_t + (size_t)t*N;
                ...
                /* u_t: hetero optional (use r_data) */
                if (U_HET_ALPHA > 0.0){
                    gen_u_het_factor(T_len, F_t, sigma_e*sigma_e, U_HET_ALPHA, &r_data, &st, u_t);
                } else {
                    gen_iid_noise(T_len, sigma_e*sigma_e, &r_data, &st, u_t);
                }
                ...
                for (int k=1; k<=steps; k++){
                    ...
                    /* gamma via univariate y ~ x (no intercept) */
                    for (int i=0;i<N;i++){ csq[i]=0.0; cxy[i]=0.0; }
                    for (int t=0;t<T_tr;t++){
                        const double *xt = X + (size_t)t*N;
                        const double yt = y_tr[t];
                    ...
                    if (current_z == N){
                        /* use Q_ols weights for prediction when z=N (same for both modes) */
                        ...
                        /* RS: either uniform or |t|-softmax sampling */
                        double acc_rs = 0.0;
                        double w_bar[128]; for (int i=0;i<N;i++) w_bar[i]=0.0;
                        double acc_Q_sub = 0.0;
                        int succ = 0;

                        /* precompute |t| only if needed */
                        double tabs[128], w_soft[128];
                        if (use_weighted){
                            compute_pred_based_tabs(X, y_tr, gamma, T_tr, N, tabs);
                            double sumw = 0.0;
                            for (int i=0;i<N;i++){
                                double val = T_ALPHA * fabs(tabs[i]);
                                if (val > 50.0) val = 50.0;      /* numeric guard */
                                w_soft[i] = exp(val);
                                sumw += w_soft[i];
                            }
                            if (sumw > 0.0){ for (int i=0;i<N;i++) w_soft[i] /= sumw; }
                        }

                        for (int rr=0; rr<K_RS; rr++){
                            if (use_weighted){
                                sample_k_weighted_no_replace(&r_sel, N, current_z, w_soft, sub_idx);
                            } else {
                                sample_k_no_replace(&r_sel, N, current_z, pool_idx, sub_idx);
                            }
                        ...
                        if (succ > 0){
                            ...
                            double sumwbar=0.0; for (int i=0;i<N;i++) sumwbar += w_bar[i];
                            if (fabs(1.0 - sumwbar) > 1e-10){
                                for (int i=0;i<N;i++) w_bar[i] /= sumwbar;   /* enforce 1'w=1 */
                            }
            ...
            printf("  z=%2d (%s): RS=%.6f  rel=%.6f%s  |  RMSE(pooled): %.6f  |  bias: %.6f  var: %.6f  |  Q_ols: %.6f  Q_rs(bag): %.6f  Q_rs(subavg): %.6f  |  L_ols: %.6f  L_rs(bag): %.6f  L_rs(subavg): %.6f\n",
                   current_z, mode_names[mode], avg_rs, rel, (current_z==N ? " [sum-to-one full]" : ""),
                   rmse_pooled_rs, bias2_rs, var_rs,
                   avg_Q_ols, avg_Q_rs_bag, avg_Q_rs_sub,
                   avg_L_ols, avg_L_rs_bag, avg_L_rs_sub);

            fprintf(fp,
                    "%d,%.6f,%.6f,%.1f,%.1f,%.1f,%d,%s,"
                    "%.6f,%.6f,%.6f,%.6f,%.6f,"
                    "%.6f,%.6f,%.6f,%.6f,"
                    "%.6f,%.6f,%.6f,"
                    "%.6f,%.6f,%.6f,"
                    ...
                    current_z, mode_names[mode],
                    rmsfe_base, avg_rs, rel,
                    rmse_pooled_base, rmse_pooled_rs,
            ...
            fflush(fp);
        } /* mode loop: UNIFORM then TSOFT */

        free(Sigma_base); free(Sigma); free(Sigma_chol);
    }
