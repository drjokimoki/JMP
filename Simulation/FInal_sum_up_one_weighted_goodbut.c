...
/* RS subset selection:
   USE_T_WEIGHTS=1 -> weighted by softmax(exp(alpha * |t|));
   USE_T_WEIGHTS=0 -> uniform. */
static const int USE_T_WEIGHTS = 1;
...
/* Compute absolute t-stats from regression y_tr ~ yhat_i (no intercept), per i.
   yhat_i,t = gamma[i] * x_{i,t} using current-origin gamma.
   t_i = b_i / se(b_i), where b_i = sum(yhat*y)/sum(yhat^2),
   se(b_i) = sqrt( sigma2_i / sum(yhat^2) ), sigma2_i = SSE_i / (T_tr - 1). */
...
/* Weighted sampling w/o replacement (Efraimidis–Spirakis).
   weights[i] >= 0. If all zero -> fallback to uniform. */
...
static void sample_k_weighted_no_replace(xrng *r, int n, int k, const double *weights, int *sub){
    double maxw = 0.0; int anypos = 0;
    for (int i=0;i<n;i++){ if (weights[i] > 0.0){ anypos = 1; if (weights[i] > maxw) maxw = weights[i]; } }
    ...
    for (int i=0;i<n;i++){
        ...
        if (w <= 0.0){
            ...
            /* E-S A-ExpJ: key_i = U_i^{1/w_i} = exp(log(U_i)/w_i) */
            double U = ((xrng_next(r) >> 8)+1.0)/16777217.0; /* (0,1] */
            keys[i].key = exp(log(U) / w);
        }
        keys[i].idx = i;
    }
            ...
            for (int repl=0; repl<n_rep; repl++){
                ...
                for (int k=1; k<=steps; k++){
                    ...
                    if (current_z == N){
                        ...
                        /* --- NEW: |t|-based softmax weights for RS sampling --- */
                        double tabs[128], w_soft[128];
                        compute_pred_based_tabs(X, y_tr, gamma, T_tr, N, tabs);
                        /* softmax with exp(alpha*|t|) (normalization not required by E-S, but fine) */
                        double maxv = tabs[0]; for (int i=1;i<N;i++) if (tabs[i]>maxv) maxv=tabs[i];
                        /* numerical stability: subtract max before exp(alpha*|t|) if desired */
                        double sumw = 0.0;
                        for (int i=0;i<N;i++){
                        ...
                        if (sumw > 0.0){ for (int i=0;i<N;i++) w_soft[i] /= sumw; } /* proper probabilities */
