...
#       (iii) sum(w)=1
#
#   + NOW: iterate over k_grid (e.g. 2:40) for EACH vintage
#
#   + OUTPUTS:
#     - summary_results: one row per (vintage, k)
#     - fin_table_all:   one row per (vintage, k, variable) with w_bar + contributions
#
...
# ---------------- One-vintage pipeline (NOW loops over k_grid) -----------------
run_forecast_kgrid <- function(monthly_path, quarterly_path,
                               n_start = 40,
                               ridge = 1e-8,
                               k_grid = 2:40,
                               verbose_checks = TRUE) {
  ...
  # Levels series (kept as in your script)
  y_levels <- tibble(
  ...
  results_mean <- map_dfr(names(X), function(varname) {
  ...
  MEAN_mean <- mean(results_mean$y_forecast_growth, na.rm = TRUE)
  ...
  # Feasible k values given available predictors in last_x_rs
  pred_vars <- names(last_x_rs)
  k_grid_eff <- sort(unique(k_grid))
  k_grid_eff <- k_grid_eff[k_grid_eff >= 1 & k_grid_eff <= length(pred_vars)]
  if (length(k_grid_eff) == 0) stop("k_grid has no feasible values for this vintage (no predictors available).")
  
  # Containers
  summary_list <- vector("list", length(k_grid_eff))
  fin_list     <- vector("list", length(k_grid_eff))
  
  for (ii in seq_along(k_grid_eff)) {
    k <- k_grid_eff[ii]
    
    # Wrap each k to avoid killing the whole vintage if one k fails
    out_k <- tryCatch({
      ...
      if (isTRUE(verbose_checks)) {
        cat("\n",
            "================ Vintage ", vintage_key, " | k=", k, " ================\n", sep = "")
        cat("COVID exclusion active? ", apply_covid_excl, "\n", sep = "")
        cat("B_succ: ", RSSS$B_succ, " / ", rs_draws, "\n", sep = "")
        cat("max |sum_w(draw)-1|: ", format(RSSS$max_abs_sumw_dev, digits = 6), "\n", sep = "")
        cat("sum(w_bar): ", format(sum_w_bar, digits = 8), "\n", sep = "")
        cat("RS_mean: ", format(RS_mean, digits = 8), "\n", sep = "")
        cat("RS_from_w_bar: ", format(RS_from_w_bar, digits = 8),
            "  |diff|=", format(abs(RS_from_w_bar - RS_mean), digits = 6), "\n", sep = "")
        cat("sum(Contribution): ", format(sum_contrib, digits = 8), "\n", sep = "")
        cat("Residual: ", format(contrib_residual, digits = 8), "\n", sep = "")
      }
      ...
      list(
        ...
        Fin_table2 = Fin_table2 %>% mutate(vintage = vintage_key, k = k)
      )
    }, error = function(e) {
      if (isTRUE(verbose_checks)) {
        cat("\n",
            "================ Vintage ", vintage_key, " | k=", k, " FAILED ================\n", sep = "")
      ...
      list(
        summary = tibble(
          ...
          RS_mean = NA_real_,
          B_succ = NA_integer_,
          max_abs_sumw_dev = NA_real_,
          sum_w_bar = NA_real_,
          RS_from_w_bar = NA_real_,
          abs_diff_RS = NA_real_,
          base_weighted = NA_real_,
          sum_contrib = NA_real_,
          contrib_residual = NA_real_,
          mean_hist_gdp = NA_real_,
          rs_vs_hist_gdp = NA_real_,
          total_vs_hist_gdp = NA_real_,
          abs_diff_vs_hist_gdp = NA_real_
        ),
        Fin_table2 = tibble(
          vintage = vintage_key, k = k,
          Var = pred_vars,
          Selected = NA_real_, mean_weight_nonzero = NA_real_, w_bar = NA_real_,
          Prediction = as.numeric(last_x_rs[1, pred_vars]),
          Hist_mean = NA_real_,
    ...
    summary_list[[ii]] <- out_k$summary
    fin_list[[ii]]     <- out_k$Fin_table2
  }
  
  list(
    summary   = bind_rows(summary_list),
    Fin_table2 = bind_rows(fin_list)
  )
}
...
# keep your existing exclusion if needed
pairs <- pairs[-18, ]

if (nrow(pairs) == 0) stop("No matched monthly/quarterly vintages found. Check file names and directories.")

# ---------------- Run across vintages (and over k_grid within each vintage) -----------------
res_list <- map2(
  pairs$monthly_path, pairs$quarterly_path,
  ~ run_forecast_kgrid(.x, .y, n_start = uni_min_start, ridge = 1e-8, k_grid = k_grid, verbose_checks = TRUE)
)

summary_results <- map_dfr(res_list, "summary")
fin_table_all   <- map_dfr(res_list, "Fin_table2")
saveRDS(res_list, file="/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_constrained/final_list_covid_all_k.RData")
...
actual_y<- tail(growth_y, n = nrow(pairs)+1)[-18,]
actual_y<-actual_y$growth_y
# actual_tbl: has Date (Date) and actual (numeric)
# Date should be quarter-dates like 2020-06-01, 2020-09-01, etc.

# actual_y: numeric vector length == nrow(pairs)
stopifnot(length(actual_y) == nrow(pairs))

actual_df <- tibble(
  vintage = pairs$vintage,
  actual  = actual_y
)

rmse_by_k <- summary_results %>%
  select(vintage, k, RS_mean) %>%      # or RS_from_w_bar
  left_join(actual_df, by = "vintage") %>%
  group_by(k) %>%
  summarise(
    n = sum(!is.na(actual) & !is.na(RS_mean)),
    rmse = sqrt(mean((RS_mean - actual)^2, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  arrange(k)
...
ggplot(rmse_by_k, aes(x = k, y = rmse)) +
  geom_line() +
  geom_point() +
  labs(x = "subset dimension", y = "RMSE") +
  theme_minimal()
...
summary_results_without_c<-summary_results[! summary_results$vintage %in% c("2020-06", "2020-09"), ]

rmse_by_k_c <- summary_results_without_c %>%
  select(vintage, k, RS_mean) %>%      # or RS_from_w_bar
  left_join(actual_df, by = "vintage") %>%
  group_by(k) %>%
  summarise(
  ...
  arrange(k)
plot(rmse_by_k_c$rmse, type = "l")

ggplot(rmse_by_k_c, aes(x = k, y = rmse)) +
  geom_line() +
  geom_point() +
  labs(x = "subset dimension", y = "RMSE") +
  theme_minimal()

aa<-readRDS("/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/IP_constrained/final_list_covid.RData")
summary_results1 <- map_dfr(aa, "summary")

aaa <- rep(NA_real_, length(res_list))
bb <- rep(NA_real_, length(res_list))

exclude <- c(1845)  # keep your exclusion
for (i in seq_along(aa)) {
  if (i %in% exclude) next
  aaa[i] <- unique(as.numeric(aa[[i]][["summary"]][["RS_mean"]]))
  bb[i] <- unique(as.numeric(aa[[i]][["summary"]][["MEAN_mean"]]))
}

summary_results$actual <- growth_y_sa$growth_y
summary_results <- data.frame(RS_mean = aaa, MEAN_mean = bb, actual = growth_y_sa$growth_y)
