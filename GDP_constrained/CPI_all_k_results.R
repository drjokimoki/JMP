...
# CPI_all_k_results.R
#
# CPI inflation forecasting — exact parallel to GDP_all_k_results.R.
# Target: CPIAUCSL quarterly growth (log q/q × 100, annualised).
# Individual forecasts: bivariate OLS with each FRED-MD predictor.
# Combination: RSM k-sweep, Lasso, Ridge, Random Forest benchmarks.
# Actuals:  latest FRED-QD vintage CPIAUCSL column.
# Sample:   103 vintages (1999-09 to 2025-03); vintage 18 (Q4-2003) excluded.
# =============================================================================
...
OUTDIR        <- file.path(WORKDIR, "GDP_constrained", "cpi_all_k_results")
...
# CPI-related series to exclude from predictors (avoid in-sample look-ahead)
CPI_EXCLUDE <- c("CPIAUCSL", "CPILFESL", "CPIAPPSL", "CPITRNSL", "CPIMEDSL",
                 "CUSR0000SAC", "CUSR0000SAS", "CUSR0000SAD",
...
cat("CPI all-k analysis started.\n")
cat("Output directory:", OUTDIR, "\n")

# ---- Helper functions (identical to GDP script) -----------------------------
...
# ---- Core vintage function --------------------------------------------------
run_forecast_allk_cpi <- function(monthly_path, quarterly_path,
  ...
  Quarterly_raw  <- Quarterly_FRED[-1, ]   # drop transformation codes row

  # ---- Monthly FRED-MD: transform, aggregate to quarterly -------------------
  Monthly_without_time <- Monthly_FRED[, -1, drop = FALSE] %>%
  ...
  Final_monthly$Date <- parse_date(Monthly_FRED[-1, 1, drop = TRUE])
  ...
  m_q <- Final_monthly %>%
    ...
    rename(Date = q_date) %>% arrange(Date) %>% slice(-1)  # drop first (incomplete)

  # ---- CPI levels from raw FRED-QD (before any transformation) --------------
  cpi_idx <- which(names(Quarterly_raw) == "CPIAUCSL")
  y_levels <- tibble(
    Date      = parse_date(Quarterly_raw[-1, 1, drop = TRUE]) %>% as_date(),
    CPI_level = suppressWarnings(as.numeric(as.character(
      Quarterly_raw[-1, cpi_idx, drop = TRUE])))
  ) %>% arrange(Date) %>% drop_na()

  # Quarterly CPI log-growth (h=1, one quarter ahead)
  y_growth_tbl <- y_levels %>%
    mutate(growth_y = log(CPI_level / dplyr::lag(CPI_level)) * 100) %>%
    filter(!is.na(growth_y)) %>%
    select(Date, growth_y)

  # ---- Predictors: monthly FRED-MD aggregated, CPI family excluded ----------
  cpi_present <- intersect(names(m_q), CPI_EXCLUDE)
  m_q_X <- m_q %>% select(-all_of(cpi_present))   # drop CPI-related series

  # ---- Join y (CPI growth) with X (monthly predictors aggregated) -----------
  estimation0 <- y_growth_tbl %>%
    inner_join(m_q_X, by = "Date") %>% arrange(Date)
  last_x      <- m_q_X %>% filter(Date == max(Date)) %>% select(-Date)

  y_last_level <- y_levels %>%
    filter(Date %in% estimation0$Date) %>% arrange(Date) %>% pull(CPI_level) %>% tail(1)
  if (length(y_last_level) == 0 || is.na(y_last_level))
    stop("Could not determine last CPI level.")
  ...
  results <- map_dfr(names(X), function(varname) {
    ...
    tibble(variable=varname, z_hat_next=z_hat_next,
           y_forecast_next = y_last_level * exp(z_hat_next / 100))
  })
  MEAN_bivariate <- mean(((results$y_forecast_next / y_last_level) - 1) * 100, na.rm = TRUE)

  # Build RS estimation dataset using univariate forecast matrix
  X_plus <- rbind(X, last_x)
  fcasts  <- univariate_forecasts_split(
    y, X_plus,
    y_levels      = y_levels %>% filter(Date %in% c(estimation0$Date, max(m_q$Date))) %>%
                      arrange(Date) %>% pull(CPI_level),
    n_start       = n_start,
  ...
  growth_y_cpi <- (y_levels$CPI_level / dplyr::lag(y_levels$CPI_level) - 1) * 100
  growth_y_cpi <- growth_y_cpi[-1]
  growth_y_df  <- data.frame(growth_y = growth_y_cpi, Date = y_levels$Date[-1])
  ...
  cat("  Vintage", vintage_key, "| k_main =", k_main,
      "| T_est =", T_est, "| n_pred =", ncol(X), "| RS =", round(RS_fore_main, 3), "\n")
...
cat("Running", nrow(pairs), "vintages for CPI (k_grid:", min(k_grid_app),
    "to", max(k_grid_app), "| B_allk =", rs_draws_allk, ")...\n")

# ---- Run all vintages -------------------------------------------------------
res_list <- map2(
  pairs$monthly_path, pairs$quarterly_path,
  ~ run_forecast_allk_cpi(.x, .y, n_start = uni_min_start, ridge = 1e-8,
                           k_grid = k_grid_app,
                           B_main = rs_draws_main, B_allk = rs_draws_allk)
)

saveRDS(res_list, file.path(OUTDIR, "res_list_allk_cpi.rds"))
cat("res_list saved.\n")
...
make_eval_table <- function(df, exclude_covid_targets = FALSE, label = "") {
  ...
  rows <- lapply(seq_along(methods), function(i) {
    ...
    tibble(Model=labels[i], MAFE=round(mafe,3), MAFE_sig=sig_stars(mafe_p),
           RMSFE=round(rmsfe,3), RMSFE_sig=sig_stars(rmsfe_p))
  })
  bind_rows(rows)
}
...
cat("\nTable 1 — CPI Forecast Evaluation: Full Sample\n"); print(tbl1, n=Inf)
cat("\nTable 2 — CPI Forecast Evaluation: Excluding COVID-19\n"); print(tbl2, n=Inf)
...
rmsfe_by_k <- function(mat, df, exclude_covid = FALSE) {
  ...
  vapply(seq_len(ncol(mat)), function(j) {
    fc  <- mat[idx, j]; act <- df$actual[idx]
...
cat(sprintf("\nFull sample:   k* = %d, RMSFE = %.3f | k~sqrt(T) = %d, RMSFE = %.3f\n",
    kstar_full,  min(rmsfe_full, na.rm=TRUE),
    median_k_main, rmsfe_full[k_vals == median_k_main]))
cat(sprintf("No-COVID:      k* = %d, RMSFE = %.3f | k~sqrt(T) = %d, RMSFE = %.3f\n",
...
make_rmsfe_fig(k_vals, rmsfe_full, kstar_full, median_k_main,
               rmsfe_full[k_vals==median_k_main],
               "CPI: RMSFE by k — Full Sample",
               file.path(OUTDIR, "fig_rmsfe_by_k_full.png"))
make_rmsfe_fig(k_vals, rmsfe_nocov, kstar_nocov, median_k_main,
               rmsfe_nocov[k_vals==median_k_main],
               "CPI: RMSFE by k — Excluding COVID-19",
               file.path(OUTDIR, "fig_rmsfe_by_k_nocovid.png"))

png(file.path(OUTDIR, "fig_rmsfe_combined.png"), width=1300, height=520, res=110)
...
for (ii in 1:2) {
  ...
  ttl    <- if (ii==1) "CPI — Full Sample" else "CPI — Excluding COVID-19"
  ylim   <- range(rmsfe,na.rm=TRUE); ylim <- ylim+c(-0.02,0.05)*diff(ylim)
  plot(k_vals, rmsfe, type="l", lwd=2.2, col="#2166AC",
...
dev.off()
cat("Saved: fig_rmsfe_combined.png\n")

cat("\nAll CPI outputs saved to:", OUTDIR, "\n")
cat("Done.\n")
