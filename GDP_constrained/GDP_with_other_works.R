...
#   + ADDITIONAL COMBINERS (NEW): LASSO, RIDGE, RANDOM FOREST
#
#   COVID OUTLIER HANDLING:
#     For vintages AFTER 2020-09, drop two COVID quarters (2020-06, 2020-09)
#     from ALL bivariate OLS estimation samples AND RS estimation dataset.
# ================================================================
...
# ================================================================
# NEW: LASSO / RIDGE / RF forecast combination helpers
#   Train on: estimation (y = growth_y, X = base forecasts columns)
#   Predict:  last_x_rs (1-row, same base-forecast columns)
# ================================================================
prep_combo_data <- function(df, y_col, x_next) {
  df <- as.data.frame(df)
  
  # accept named vector or 1-row df for x_next
  if (is.vector(x_next)) {
    if (is.null(names(x_next))) stop("x_next must be named if provided as vector.")
    x_next <- as.data.frame(as.list(x_next))
  } else {
    x_next <- as.data.frame(x_next)
  }
  ...
  cand <- cand[!is.na(as.numeric(x_next[1, cand]))]
  if (length(cand) < 1) stop("No usable predictors (overlap + non-NA in x_next).")
  
  X <- X_all[, cand, drop = FALSE]
  xrow <- x_next[, cand, drop = FALSE]
  
  # remove zero-variance columns
  nzv <- sapply(X, function(z) {
    z <- z[is.finite(z)]
  ...
  cand <- cand[nzv]
  X <- X[, cand, drop = FALSE]
  xrow <- xrow[, cand, drop = FALSE]
  
  cc <- stats::complete.cases(y, X)
  y <- y[cc]
  X <- X[cc, , drop = FALSE]
  
  list(y = y, X = X, xrow = xrow, vars = cand)
}

glmnet_combo_forecast <- function(df, y_col, x_next,
  ...
  if (n < 8) {
    ...
    return(list(forecast = pred, model = fit, lambda = lam, coefs = co, vars = dat$vars))
  }
  ...
  list(forecast = pred, model = cvfit, lambda = lam, coefs = co, vars = dat$vars)
}

rf_combo_forecast <- function(df, y_col, x_next,
                              num_trees = 500,
                              mtry = NULL,
                              min_node_size = 5,
                              seed = 123) {
  ...
  list(forecast = pred, model = rf, vars = dat$vars)
}
...
run_forecast <- function(monthly_path, quarterly_path, n_start = 40, ridge = 1e-8, verbose_checks = TRUE) {
  ...
  # GDP levels for back-transform (raw levels)
  y_levels <- tibble(
  ...
  # Keep GDPC1 + Date in transformed quarterly
  ...
  # ========= MEAN comb step (your existing bivariate OLS mean benchmark) =========
  y <- estimation0$GDPC1
  ...
  # mean-of-experts benchmark at the forecast date (this is the mean of columns in last_x_rs)
  ...
  # ========= NEW: LASSO / RIDGE / RANDOM FOREST combination =========
  LASSO_fore <- tryCatch({
    glmnet_combo_forecast(estimation, "growth_y", last_x_rs, alpha = 1, lambda_choice = "lambda.1se")$forecast
  }, error = function(e) NA_real_)
  
  RIDGE_fore <- tryCatch({
    glmnet_combo_forecast(estimation, "growth_y", last_x_rs, alpha = 0, lambda_choice = "lambda.1se")$forecast
  }, error = function(e) NA_real_)
  ...
  if (isTRUE(verbose_checks)) {
    ...
    cat("Extra combiners at target_date = ", as.character(target_date), ":\n", sep = "")
    cat("  MEAN_fore_rs = ", format(MEAN_fore_rs, digits = 8), "\n", sep = "")
    cat("  LASSO_fore   = ", format(LASSO_fore, digits = 8), "\n", sep = "")
    cat("  RIDGE_fore   = ", format(RIDGE_fore, digits = 8), "\n", sep = "")
    cat("  RF_fore      = ", format(RF_fore, digits = 8), "\n", sep = "")
  }
  ...
  list(
    summary = tibble(
      ...
      MEAN_mean = MEAN_mean,        # your original mean benchmark from bivariate OLS growth forecasts
      MEAN_fore_rs = MEAN_fore_rs,  # mean of base-forecast columns at forecast date (clean "mean-of-experts" baseline)
...
# Optional exclusion of a problematic index (keep your previous behavior)
exclude_pair_idx <- c(18)
...
#saveRDS(res_list, file="/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/GDP_constrained/final_list_covid_other_methods.RData")

#res_list<- readRDS("/Users/boris/Desktop/RSShapley/Combination_Monte/Final/codess/JMP/GDP_constrained/final_list_covid_other_methods.RData")
...
qq<- rep(NA_real_, length(res_list))
da<-rep(NA_real_, length(res_list))
das<- rep(NA_real_, length(res_list))
...
for (i in seq_along(res_list)) {
  ...
  qq[i] <- unique(as.numeric(res_list[[i]][["summary"]][["LASSO_fore"]]))
  da[i] <- unique(as.numeric(res_list[[i]][["summary"]][["RIDGE_fore"]]))
  das[i] <- unique(as.numeric(res_list[[i]][["summary"]][["RF_fore"]]))
}

summary_results$actual <- growth_y_sa$growth_y
summary_results <- data.frame(RS_mean = aa, MEAN_mean = bb, LASSO= qq, Ridge= da, RF=das, actual = growth_y_sa$growth_y)
...
mean(abs(summary_results$Ridge - summary_results$actual), na.rm = TRUE)
sqrt(mean((summary_results$Ridge - summary_results$actual)^2, na.rm = TRUE))
...
mean(abs(summary_results$Ridge[-c(83,84)]   - summary_results$actual[-c(83,84)]), na.rm = TRUE)
sqrt(mean((summary_results$Ridge[-c(83,84)] - summary_results$actual[-c(83,84)])^2, na.rm = TRUE))



e2 <- summary_results$MEAN_mean - summary_results$actual
e1 <- summary_results$RF   - summary_results$actual

dm.test(e1, e2, h = 1, alternative = "less", power = 2)
# ================================================================
# OPTIONAL: Ex-post evaluation (RMSE) by merging realized growth
# from the latest quarterly vintage onto target_date.
#
# This is usually what you want in pseudo-real-time:
# forecasts are for t+1, realized is only known in later vintages.
# ================================================================

e2 <- summary_results$MEAN_mean[-c(83,84)] - summary_results$actual[-c(83,84)]
e1 <- summary_results$LASSO[-c(83,84)]  - summary_results$actual[-c(83,84)]

dm.test(e1, e2, h = 1, alternative = "less", power = 1)
